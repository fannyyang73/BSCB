...
results <- foreach(i = 1:replication,  .packages = c("mvtnorm","MASS","glmnet","cmdstanr", "posterior")) %dopar% { #, "caret"
...
if(HMC_setting == "1"){
  ...
  worker_mod <- cmdstan_model("HMC_p=2.stan", compile = TRUE, force_recompile = FALSE)
  ...
  # theta_quantiles: 2 x 3 matrix, rows = [lower, upper], cols = theta[1..3]
  rownames(theta_quantiles) <- c(paste0("Q", delta/2*100, "%"), paste0("Q", (1-delta/2)*100, "%"))
  colnames(theta_quantiles) <- c("theta1", "theta2", "theta3")
...
if(HMC_setting == 1){
  theta_q_df <- do.call(rbind, lapply(seq_along(theta_quantiles_list), function(i){
    q <- theta_quantiles_list[[i]]
    data.frame(rep = i, quantile = rownames(q),
               theta1 = q[,"theta1"], theta2 = q[,"theta2"], theta3 = q[,"theta3"])
  }))
  write.csv(theta_q_df, "theta_posterior_quantiles.csv", row.names = FALSE)
}
