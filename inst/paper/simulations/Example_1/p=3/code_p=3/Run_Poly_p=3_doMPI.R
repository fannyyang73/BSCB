...
Sys.setenv(CMDSTAN = "/mnt/iusers01/maths01/w85950fy/.cmdstan/cmdstan-2.37.0")
...
HMC_setting <- 1 # 0: Use the normal-gamma prior; 1:Use HMC; 2: Use the non-informative prior; 3: Use the independent Jefferys prior
hyper_prior_setting <- 1 # 0: empirical ; 1: unit-information; 2: g-prior
...
########################       Parallel Computing       ########################
multiResultClass <- function(i=NULL, lambda_best_optim=NULL, coverage_flag=NULL, mu_star=NULL, cov_theta=NULL, coverage_prob=NULL, coverage_draw=NULL, seed_used=NULL, elapsed_time=NULL,theta_quantiles=NULL, theta_mat=NULL){
  me <- list(i = i, lambda_best_optim = lambda_best_optim, coverage_flag = coverage_flag, mu_star = mu_star,
             cov_theta=cov_theta, coverage_prob = coverage_prob, coverage_draw = coverage_draw, seed_used = seed_used, elapsed_time = elapsed_time,theta_quantiles = theta_quantiles, theta_mat=theta_mat)
...
start_time <- Sys.time()
result_mat <- matrix(NA, nrow = replication, ncol = 7)
...
model_path <- "/mnt/iusers01/maths01/w85950fy/SCB/10000/HMC_p=3.stan"
if(HMC_setting == 1){
  Sys.setenv(CMDSTAN = "/mnt/iusers01/maths01/w85950fy/.cmdstan/cmdstan-2.37.0")
  mod_precompile <- cmdstan_model(model_path, force_recompile = TRUE)
  cat("Pre-compilation done:", mod_precompile$exe_file(), "\n")
}
...
results <- foreach(i = 1:replication,
                   .packages = c("mvtnorm","MASS","glmnet","cmdstanr","posterior"),
                   .export = c("X","Y.list","n","p","L","a","b","delta","alpha",
                               "V","V_inv","HMC_setting","hyper_prior_setting",
                               "theta_true","draw_num","x_setting","order_form",
                               "tr","rmvt","f_L_SCB","f_U_SCB","L_SCB","U_SCB",
                               "prob_x_SCB","integrand_SCB","integrand_SCB_HMC",
                               "find_global_maximum","fn_Bayes_PCP","fn_Bayes_ECR",
                               "replication_seeds","compute_NG_param",
                               "model_path","cauchy_scale",
                               "HMC_prior_type","normal_theta_sd","normal_sigma_sd",
                               "find_global_maximum_h_all","sup_T_Bayes_PSCP","sup_T_Bayes_ESCR","sup_T_simFSCB","sup_T_Freq_ESCR")) %dopar% {
                                 ...
                                 if(HMC_setting == "1"){
                                   ...
                                   worker_outdir <- file.path(
                                     "/mnt/iusers01/maths01/w85950fy/scratch",
                                     Sys.getenv("SLURM_JOB_ID", "local"),
                                     paste0("worker_", i)   # 每个 iteration 独立子目录
                                   )
                                   dir.create(worker_outdir, recursive = TRUE, showWarnings = FALSE)

                                   Sys.setenv(CMDSTAN = "/mnt/iusers01/maths01/w85950fy/.cmdstan/cmdstan-2.37.0")
                                   worker_mod <- cmdstan_model(model_path, compile = TRUE)
                                   # worker_mod <- cmdstan_model("HMC_p=2.stan", compile = TRUE)
                                   ...
                                   # Sampling to compute lambda_sup
                                   for (j in 1:L) {
                                   ...
                                   S <- nrow(theta_mat)
                                   SS <- 1000 # the number of discrete x in a given range
                                   if(x_setting == "1"){
                                     #x_vals <- runif(S, min = a, max = b)
                                     x_vals <- seq(from = a, to = b, length.out = SS)
                                   }else if(x_setting == "2"){x_vals <- rnorm(S, mean = 0, sd = 1)}

                                   coverage_prob_sum <- 0
                                   XTtheta_samples <- matrix(nrow = S, ncol = 1)
                                   for(z in 1:SS){
                                     x_val <- x_vals[z]
                                     X_vec <- order_form(x_val)
                                     for(zz in 1:S){
                                       XTtheta_samples[zz, ] <- t(X_vec)%*%theta_mat[zz,]
                                     }
                                     ecdf_XTtheta <- ecdf(XTtheta_samples)
                                     coverage_prob_z <- integrand_SCB_HMC(x_val)
                                     dx <- (b-a)/SS
                                     coverage_prob_sum <- coverage_prob_sum + coverage_prob_z * dx
                                   }
                                   coverage_prob <- coverage_prob_sum
                                   cat("The coverage probability of Bayesian simultaneous credible band for Round", i , "is:", coverage_prob, "\n")
                                   ...
                                   ###################.  Bayesian Posterior Probability. #####################
                                   coverage_prob <- integrate(Vectorize(integrand_SCB), lower = a, upper = b)$value
                                   cat("The coverage probability of Bayesian simultaneous credible band for Round", i , "is:", coverage_prob, "\n")
                                 ...
                                 result$coverage_prob <- coverage_prob
...
for(i in 1:replication){
  ...
  result_mat[i,4] <- results[[i]]$coverage_prob
  result_mat[i,5] <- results[[i]]$coverage_draw
  result_mat[i,6] <- results[[i]]$seed_used
  result_mat[i,7] <- results[[i]]$elapsed_time

  mu_star_mat[i,] <- results[[i]]$mu_star
  cov_theta_list[[i]] <- results[[i]]$cov_theta
  coverage_prob_list[[i]] <- results[[i]]$coverage_prob
...
colnames(result_mat) <- c("Index", "Lambda_best_optim", "Empirical_Coverage_Rate","Average_Coverage_Probability", "Coverage_Rate_Draw","Seed_used","Elapsed_time (seconds)")

# Count success cases
sucess_count_SCB <- sum(result_mat[, 3])
...
SCB_avg_cov_prob <- mean(result_mat[,4])  # Average Coverage Probability
SCB_avg_cov_draw <- mean(result_mat[,5])  # Posterior Coverage Rate

ac_mat <- t(replicate(replication, theta_true))
MSE <- mean(rowSums((ac_mat-mu_star_mat)^2))

final_result <- matrix(nrow=1, ncol = 6)
final_result[1,1] <- N_index
final_result[1,2] <- SCB_emp_cov_rate
final_result[1,3] <- SCB_avg_cov_prob
final_result[1,4] <- SCB_avg_cov_draw
final_result[1,5] <- MSE
final_result[1,6] <- duration_cov
colnames(final_result) <- c("Batch_index", "SCB_emp_cov_rate","SCB_avg_cov_prob", "SCB_avg_cov_draw","MSE", "Total Time")
...
cat("The Average Coverage Probability (by calculation) for the Bayesian simultaneous credible band is:", SCB_avg_cov_prob, "\n")
