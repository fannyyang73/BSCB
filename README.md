...
The goal of BSCB is to quantify the uncertainty in an unknown regression
functions over a given continuous range. The method consists of This
manuscript proposes a novel framework for constructing a $(1-\alpha)$
two-sided exact BSCB for the polynomial model over the given covariate
interval $(a,b)$. Three approaches have been developed based on (i) the
normal-gamma conjugate prior, (ii) the independent Jeffreys prior for
objective inference, and (iii) the non-conjugate prior implemented by
...
A full demo for how to use the R package BSCB is provided in
[here](https://github.com/fannyyang73/BSCB/tree/main/demo/BSCB_demo.Rmd).

You can install the development version of BSCB from
